      ...
      <PopoverContent className="w-64 max-h-80 overflow-auto p-2">
        {options.length === 0 ? (
          ...
          <div className="space-y-1">
            {selected.length > 0 && (
              <button
                className="text-xs text-primary hover:underline mb-1 px-2"
                onClick={() => onChange([])}
              >
                Limpar
              </button>
...
function Relatorios() {
    ...
    queryFn: async () => {
      ...
      while (from < 500_000) {
        const { data, error } = await supabase
          ...
          .select(
            "invoice_date, revenue, volume_sales, representative, rep_code, client_name, client_id, product_name, line, product_group, state, city, invoice_number",
          )
  ...
  const filtered = useMemo(
    () => (allSales ?? []).filter(matchesFilters),
    [allSales, selReps, selStates, selLines],
  );
  ...
  const previous = useMemo(
    () =>
      filtered.filter(
        (s) => s.invoice_date && s.invoice_date >= prevStart && s.invoice_date < periodStart,
      ),
  ...
  const invoiceCount =
    new Set(current.map((s) => s.invoice_number).filter(Boolean)).size || current.length;
  const activeClients = new Set(current.map((s) => s.client_id ?? s.client_name).filter(Boolean))
    .size;
  ...
  function exportMonthlyCSV() {
    ...
    a.href = url;
    a.download = `relatorio-mensal-${period}m.csv`;
    a.click();
  ...
  function exportDetailedCSV() {
    ...
    const rows = current
      .map((s) =>
        [
          s.invoice_date,
          s.client_name,
          s.representative ?? s.rep_code,
          s.line ?? s.product_group,
          s.product_name,
          s.state,
          s.city,
          Number(s.volume_sales ?? 0),
          Number(s.revenue ?? 0),
        ]
          .map(esc)
          .join(","),
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `relatorio-detalhado-${period}m.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Análise de vendas, representantes, clientes, linha e UF.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          {yoy.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Comparativo mensal — atual vs ano anterior</CardTitle>
              </CardHeader>
              <CardContent className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={yoy}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card>
              <CardHeader>
                <CardTitle>Top 10 representantes</CardTitle>
              </CardHeader>
                  ...
                  >
                    ...
                    <Tooltip
                      formatter={(v: any, _n, p: any) => [
                        `R$ ${Number(v).toLocaleString("pt-BR")} (${p.payload.pct.toFixed(1)}%)`,
                        "Receita",
                      ]}
                    />
          ...
          <p className="text-xs text-muted-foreground">
            Dados agregados a partir de {current.length.toLocaleString("pt-BR")} linhas de venda no
            período{hasFilters ? " (com filtros aplicados)" : ""}. Volume em kg.
          </p>
        </>
      )}

      <DrillDownSheet drill={drill} onClose={() => setDrill(null)} rows={current} period={period} />
    </div>
  );
}
...
function DrillDownSheet({
  ...
  const invoices =
    new Set(filtered.map((r) => r.invoice_number).filter(Boolean)).size || filtered.length;
      ...
      <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto p-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
          <DrillKpiCard
            icon={DollarSign}
            label="Faturamento"
            value={`R$ ${formatCurrencyCompact(totalRevenue)}`}
          />
          <DrillKpiCard icon={Receipt} label="Notas" value={invoices.toLocaleString("pt-BR")} />
          <DrillKpiCard
            icon={TrendingUp}
            label="Ticket médio"
            value={`R$ ${formatCurrencyCompact(avgTicket)}`}
          />
          ...
          <Table>
            ...
            <TableBody>
              {sorted.slice(0, 200).map((r, i) => (
                <TableRow
                  key={`${r.invoice_number ?? "nf"}-${r.client_id ?? r.client_name ?? "cli"}-${r.invoice_date ?? "dt"}-${i}`}
                >
                  <TableCell className="whitespace-nowrap text-xs">
                    {formatDateBR(r.invoice_date)}
                  </TableCell>
                  ...
                  <TableCell className="text-right text-xs whitespace-nowrap">
                    R${" "}
                    {Number(r.revenue ?? 0).toLocaleString("pt-BR", {
